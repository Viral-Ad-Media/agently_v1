...
export default function CallSimulator({ isOpen, onClose }: CallSimulatorProps) {
  ...
  const [scenario, setScenario] = useState('I want to schedule an appointment and need a callback.');
  ...
  const clearTimers = () => {
    [timerRef, connectTimeoutRef, transferTimeoutRef, closeTimeoutRef].forEach((ref) => {
      if (ref.current) { clearInterval(ref.current as any); clearTimeout(ref.current as any); ref.current = null; }
    });
  };
  ...
  const initiateWebCall = async () => {
    ...
    try {
      ...
      setWebCallError('Microphone access denied. Allow mic access to make calls.');
    ...
    try {
      ...
      setTimeout(() => {
        ...
        webCallTimerRef.current = setInterval(() => setWebCallDuration((d) => d + 1), 1000);
      }, 500);
    } catch {
      setTimeout(() => {
        setWebCallStatus('connected');
  ...
  const startCall = () => {
    ...
    connectTimeoutRef.current = setTimeout(async () => {
      ...
      setDuration(0);
      setIntent('Greeting');
      setMessages([{ speaker: 'Agent', text: agent?.greeting || 'Hello, how can I help you today?' }]);
  ...
  const handleTransfer = () => {
    ...
    transferTimeoutRef.current = setTimeout(() => { void endCall({ outcome: CallOutcome.ESCALATED }); }, 3000);
  };

  const endCall = async (options?: { outcome?: CallOutcome }) => {
    if (timerRef.current) { clearInterval(timerRef.current); timerRef.current = null; }
    setStatus('summarizing');
    const transcriptMessages = messages.length > 0 ? messages : [
      { speaker: 'Agent' as const, text: agent?.greeting || 'Hello!' },
      { speaker: 'You' as const, text: scenario },
    ];
    await handleSimulatorFinished({
      transcript: transcriptMessages.map((m) => `${m.speaker}: ${m.text}`).join('\n'),
  ...
  useEffect(() => {
    if (status === 'active' && duration > 5) setIntent('Inquiry about Services');
    if (duration > 15) setIntent('Lead Information');
  }, [duration, status]);

  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => { if (e.key === 'Escape') { clearTimers(); onClose(); } };
  ...
  return createPortal(
    <div
      ...
      onMouseDown={(e) => { if (e.target === e.currentTarget) { clearTimers(); onClose(); } }}
    >
      <div
      ...
      >
        ...
        <div className="bg-ink-950 px-7 py-5 text-white flex items-center justify-between flex-shrink-0">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-brand-orange">Agent Test Console</p>
            <h3 className="text-xl font-black tracking-tight mt-0.5">{agent?.name ?? 'Voice Agent'}</h3>
            <p className="text-xs text-white/40 mt-0.5 font-mono">{agent?.twilioPhoneNumber || 'No number assigned'}</p>
          </div>
          <button onClick={onClose} className="p-2.5 hover:bg-white/10 rounded-2xl transition-all text-white/60 hover:text-white">
            ✕
          </button>
        </div>
            ...
            <div className="p-6 space-y-5">
              {!hasNumber && (
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                  <p className="text-sm font-bold text-amber-800">No number assigned to this agent</p>
                  <p className="text-xs text-amber-600 mt-1">Go to Phone Numbers to assign a Twilio number first.</p>
                </div>
              )}
              {webCallStatus === 'idle' && (
                <>
                  <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Make a Real Call</p>
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Your Name</label>
                        <input value={callerName} onChange={(e) => setCallerName(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-orange" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Your Phone</label>
                        <input value={callerPhone} onChange={(e) => setCallerPhone(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-orange" />
                      </div>
                    </div>
                    {webCallError && <p className="text-xs text-red-500 font-medium mb-3">{webCallError}</p>}
                    <button
              ...
              {webCallStatus === 'connecting' && (
                <div className="flex flex-col items-center py-10 text-center">
                  <div className="w-20 h-20 rounded-[2rem] bg-brand-orange flex items-center justify-center shadow-xl mb-6 animate-pulse text-4xl">📞</div>
                  <p className="text-xl font-black text-ink-900 mb-1">Connecting…</p>
                  <p className="text-sm text-slate-400">Linking browser to {agent?.twilioPhoneNumber}</p>
                </div>
              )}
              {webCallStatus === 'connected' && (
                <div className="space-y-4">
                  <div className="flex flex-col items-center py-6 text-center">
                    <div className="w-20 h-20 rounded-full bg-ink-900 flex items-center justify-center shadow-2xl mb-4 text-3xl">🎙️</div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                      <p className="text-emerald-600 font-black text-sm uppercase tracking-widest">Live Call Active</p>
                    </div>
                    <p className="text-3xl font-black text-ink-900 tracking-tight">{formatTime(webCallDuration)}</p>
                    <p className="text-xs text-slate-400 mt-1">{agent?.name} is listening</p>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <button className="rounded-2xl border border-slate-200 bg-slate-50 text-slate-600 py-3.5 flex flex-col items-center gap-1.5 hover:border-slate-300 transition-all">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest">Speaker</span>
                    </button>
                    <button
                      onClick={endWebCall}
              ...
              {webCallStatus === 'ended' && (
                <div className="flex flex-col items-center py-10 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center mb-4 text-3xl">✅</div>
                  <p className="text-xl font-black text-ink-900 mb-1">Call Ended</p>
                  <p className="text-sm text-slate-400">Duration: {formatTime(webCallDuration)} · Lead data saved</p>
                </div>
              )}
            </div>
          )}
          ...
          {mode === 'sim' && (
            <div className="p-6">
              {status === 'idle' && (
                <div className="space-y-4">
                  <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Simulate Caller Info</p>
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Caller Name</label>
                        <input value={callerName} onChange={(e) => setCallerName(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-orange" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Caller Phone</label>
                        <input value={callerPhone} onChange={(e) => setCallerPhone(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-orange" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Caller Request / Scenario</label>
                      <textarea value={scenario} onChange={(e) => setScenario(e.target.value)} rows={3} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium outline-none resize-none focus:ring-2 focus:ring-brand-orange" />
                    </div>
                  </div>
                  <button
                    onClick={startCall}
                  ...
                  >
                    ...
                    {agent?.direction === 'outbound' ? 'Launch Outbound Simulation' : 'Initiate Inbound Simulation'}
                  </button>
                </div>
              )}
              {status === 'calling' && (
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  <div className="w-16 h-16 bg-ink-900 text-white rounded-[1.5rem] flex items-center justify-center mb-5 text-3xl animate-pulse">📲</div>
                  <p className="text-xl font-black text-ink-900">{agent?.direction === 'outbound' ? 'Dialing…' : 'Ringing…'}</p>
                  <p className="text-xs text-slate-400 uppercase tracking-widest mt-1">Connecting to {agent?.direction} workflow</p>
                </div>
              )}
              {status === 'active' && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center bg-slate-50 border border-slate-200 rounded-2xl p-4">
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Detected Intent</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-black text-ink-900 text-xl">{formatTime(duration)}</p>
                    </div>
                  </div>
                  <div className="rounded-2xl bg-ink-900 p-4 space-y-2 max-h-48 overflow-y-auto">
                    {messages.map((msg, i) => (
                      <div key={i} className={`flex ${msg.speaker === 'You' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] px-3 py-2 rounded-xl text-sm font-medium ${msg.speaker === 'You' ? 'bg-brand-orange text-white' : 'bg-white/10 text-white/90'}`}>
                          <p className="text-[10px] font-black uppercase tracking-widest mb-1 opacity-60">{msg.speaker}</p>
                          {msg.text}
                        </div>
                      </div>
                    ))}
              ...
              {status === 'transferring' && (
                <div className="flex flex-col items-center py-10 text-center">
                  <div className="w-16 h-16 bg-amber-50 text-amber-600 rounded-full flex items-center justify-center mb-5 border-2 border-amber-200 text-3xl">📤</div>
                  <p className="text-xl font-black text-ink-900">Transferring to Human…</p>
                  <p className="text-xs text-slate-400 mt-1 uppercase tracking-widest">Via {agent?.escalationPhone}</p>
                </div>
              )}
              {status === 'summarizing' && (
                <div className="flex flex-col items-center py-10 text-center">
                  <div className="w-12 h-12 border-4 border-brand-orange border-t-transparent rounded-full animate-spin mx-auto mb-5" />
                  <p className="text-xl font-black text-ink-900">Processing Outcome…</p>
                  <p className="text-xs text-slate-400 mt-1">Extracting lead data and intent</p>
                </div>
              )}
            </div>
          )}
