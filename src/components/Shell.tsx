...
import {
  LayoutDashboard, Mic2, Hash, MessageSquare, PhoneCall, Send, Bell,
  Users, CreditCard, Settings, LogOut, Menu, ChevronRight, TrendingUp,
  ShieldCheck, Play
} from 'lucide-react';
...
export default function Shell() {
  ...
  const { workspace, showSimulator, setShowSimulator, handleLogout } = useWorkspace();
  ...
  const usagePercent = usage
    ? Math.min(100, Math.round((usage.minutes / (usage.minuteLimit || 1)) * 100))
    : 0;
              ...
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.name}
                    {isActive && <ChevronRight className="ml-auto h-4 w-4 text-white/20" />}
                  </Link>
                );
              })}
            </div>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-xs mb-1.5">
                      ...
                      <span className="text-white">{usage?.minutes?.toLocaleString() ?? 0} / {usage?.minuteLimit?.toLocaleString() ?? 0}</span>
                    </div>
          ...
          <div className="p-4 border-t border-white/5">
            {user && (
              <div className="flex items-center gap-3 px-2 py-2 mb-2">
                <div className="h-8 w-8 rounded-lg bg-ink-800 border border-white/10 overflow-hidden flex-shrink-0">
                  <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name}`} alt={user.name} referrerPolicy="no-referrer" />
                </div>
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-3 w-full px-4 py-3 text-sm text-white/40 hover:text-white hover:bg-white/5 rounded-luxury transition-colors"
            >
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex items-center gap-3 mr-4">
              ...
              <div className="px-3 py-1.5 rounded-full bg-brand-orange/10 text-brand-orange text-[10px] font-bold uppercase tracking-wider border border-brand-orange/20">
                {org?.voiceAgents?.length ?? 0} agents
              </div>
            </div>

            <Link to="/notifications" className="relative p-2 text-slate-lux hover:text-ink-900 transition-colors">
            ...
            <div className="h-10 w-10 rounded-xl bg-ink-900 border border-white/10 flex items-center justify-center p-0.5 overflow-hidden">
              <img
                src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.name ?? 'Agently'}`}
                alt={user?.name ?? 'User'}
