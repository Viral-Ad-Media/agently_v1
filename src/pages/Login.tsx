...
import { ShieldCheck, Mail, Lock, ArrowRight, Sparkles, Globe, Zap, User, Building2 } from 'lucide-react';
...
export default function Login() {
  ...
  const { handleLogin, handleRegister, handleSendMagicLink } = useWorkspace();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (useMagicLink) {
        ...
        await handleRegister({ name, companyName, email, password });
        navigate('/onboarding');
      }
    } catch (err) {
      ...
      <div className="hidden lg:flex w-1/2 bg-ink-950 relative overflow-hidden flex-col p-16 justify-between border-r border-white/5">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-40">
          <div className="relative h-96 w-96">
            <motion.div
              animate={{ scale: [1, 1.1, 1], rotate: [0, 90, 180, 270, 360], opacity: [0.3, 0.5, 0.3] }}
        ...
        <div className="w-full max-w-md space-y-10">
          <div className="space-y-3">
            ...
            <p className="text-slate-lux">
              {magicLinkSent
                ? `We sent a secure link to ${email}. Click it to sign in.`
                : isLogin
            ...
            <>
              <div className="flex p-1 bg-slate-100 rounded-luxury">
                <button
                  ...
                  className={cn('flex-1 py-2 text-sm font-semibold rounded-luxury transition-all', isLogin ? 'bg-white text-ink-950 shadow-sm' : 'text-slate-lux hover:text-ink-950')}
                >
                  Sign In
                </button>
                <button
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                  ...
                  {!isLogin && !useMagicLink && (
                    <>
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-lux px-1">Full Name</label>
                        <div className="relative">
                          <User className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-lux/40" />
                          <input
                            type="text"
                            required
                            value={name}
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-lux px-1">Company Name</label>
                        <div className="relative">
                          <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-lux/40" />
                          <input
                            type="text"
                            required
                            value={companyName}
                  ...
                  {!useMagicLink && (
                    <div className="space-y-2">
                      <div className="flex justify-between items-center px-1">
                        ...
                        {isLogin && <button type="button" className="text-[10px] font-bold text-brand-orange hover:text-brand-orange/80 uppercase tracking-widest">Forgot?</button>}
                      </div>
                      <div className="relative">
                ...
                <div className="space-y-4">
                  <LuxuryButton type="submit" disabled={loading} className="w-full py-4 h-14 text-base shadow-xl shadow-brand-orange/10 group" variant="primary">
                    {loading ? (
                      <div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent" />
                    ) : (
                      <>
                        {useMagicLink ? 'Send Magic Link' : isLogin ? 'Sign In to Workspace' : 'Create Account'}
          ...
          {magicLinkSent && (
            <div className="text-center space-y-4">
              <div className="w-16 h-16 rounded-2xl bg-brand-orange/10 flex items-center justify-center mx-auto text-3xl">📧</div>
              <button onClick={() => setMagicLinkSent(false)} className="text-sm text-brand-orange font-semibold hover:underline">
